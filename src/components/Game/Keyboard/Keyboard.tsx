...
export function Keyboard({ lang }: KeyboardProps) {
  ...
  const [keyLayout, setKeyLayout] = useState('ru');
    ...
    <div className={styles.Keyboard}>
      <div className={styles.switchButtons}>
        ...
        <button
          type="button"
          className={styles.hideShowKeyboardBtn}
          onClick={onClicksKeyLayoutBtnHandler}
        >
          <RiTranslate className={styles.btnIcons} />
          {keyLayout === 'ru' ? keyLayoutBtn.ru : keyLayoutBtn.en}
        </button>
      </div>
      <div className={showKeyboard ? styles.keyboardImg__active : styles.keyboardImg}>
        {keyLayout === 'ru' ? KeyboardSvgRu : KeyboardSvgEn}
      </div>
    </div>
  );
}
